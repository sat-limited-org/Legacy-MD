...
const isSudo = (sender) => {
  ...
  return config.sudoNumbers.some(sudo => {
    const normalizedSudo = normalizeJidWithLid(
      sudo.includes('@') ? sudo : `${sudo}@s.whatsapp.net`
    );
    const sudoNumber = normalizeJid(normalizedSudo);
    return sudoNumber === senderNumber;
  });
};
